...
class ChannelManager:
    ...
    async def _publish_startup_triggers(self) -> None:
        """Publish a synthetic inbound per channel on service start.

        Channels override BaseChannel.build_startup_trigger() to return an
        InboundMessage. Default is None (skip). Failures are swallowed so a
        bad trigger never takes down service startup.
        """
        for name, channel in self.channels.items():
            try:
