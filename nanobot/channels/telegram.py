...
#
#   def my_hook(msg: OutboundMessage) -> None:
#       ...  # inspect msg.content, msg.chat_id, msg.metadata
#
#   register_outbound_hook(my_hook)
#
# Hooks are sync, fire-and-forget, best-effort: exceptions are caught and
# debug-logged. The default hook logs error-shaped output so the AI can read
# errors in the agent log and decide what to do.

_OUTBOUND_HOOKS: list[Callable[["OutboundMessage"], None]] = []

# Narrow set of patterns matching nanobot's actual error emission paths:
#   "Error:"         — runner.py, loop.py, subagent.py, provider fallbacks
#   "Error calling"  — providers/* outer-exception branch
...
def register_outbound_hook(hook: Callable[["OutboundMessage"], None]) -> None:
    ...
    Hooks fire after the message is queued for delivery, are sync,
    best-effort (exceptions are caught and debug-logged), and skip progress
    updates. Use for mail dispatch, audit logging, side-channel notifications.
    """
...
def _run_outbound_hooks(msg: "OutboundMessage") -> None:
    ...
    for hook in _OUTBOUND_HOOKS:
        try:
            ...
            logger.debug("outbound hook {} failed: {}", getattr(hook, "__name__", hook), exc)
...
class TelegramConfig(Base):
    ...
    # Matron: when non-empty, only respond to messages in these forum topics.
    allowed_thread_ids: list[int] = Field(default_factory=list)
    # Matron: where to publish the synthetic "[system] You just started." message
    # on service start. If startup_chat_id is empty, no trigger fires.
    startup_chat_id: str = ""
...
class TelegramChannel(BaseChannel):
    ...
    def build_startup_trigger(self) -> InboundMessage | None:
        """Matron: synthesize a service-start inbound so the agent runs a turn on boot.

        Fires only when startup_chat_id is set. The agent receives a minimal
        '[system] You just started.' message and decides what to do based on
        its own SOUL/handbook (e.g. re-read the handbook, post a readiness
        check-in).
        """
        ...
        if self.config.startup_thread_id is not None:
            ...
            # Match _derive_topic_session_key so the startup turn lands in the
            # right topic-scoped session.
            session_key_override = (
                f"telegram:{chat_id}:topic:{self.config.startup_thread_id}"
            )
    ...
    async def _download_message_media(
        ...
        try:
            ...
            if media_type == "image":
                # Matron: route through image_placeholder_text which OCRs when
                # the agent's model has no vision support.
                from nanobot.utils.helpers import image_placeholder_text
                return [path_str], [image_placeholder_text(path_str)]
            return [path_str], [f"[{media_type}: {path_str}]"]
        except Exception as e:
    ...
    async def _is_group_message_for_bot(self, message) -> bool:
        ...
        # Matron topic filter: if allowed_thread_ids is set, reject anything
        # outside the listed forum topics (including messages without a thread).
