...
def _run_gateway(
    ...
    *,
    ...
    async def on_heartbeat_deliver(tasks: str) -> None:
        """Matron: deliver heartbeat as a synthetic InboundMessage on the bus.

        This bypasses the shadow-session path (on_execute/on_notify) and
        instead injects the wake nudge as a normal inbound into the agent's
        real session, so the agent processes it with full conversation
        context and any reply is naturally part of that thread.
        """
    ...
    heartbeat = HeartbeatService(
        ...
        on_deliver=on_heartbeat_deliver if getattr(hb_cfg, "use_deliver", True) else None,
